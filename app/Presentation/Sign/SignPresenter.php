...
final class SignPresenter extends BasePresenter
{
     ...
     */
    public function handleCheckEmail(): void
    {
        $email = $this->getParameter('email');
        $exists = $email ? $this->userRepository->emailExists($email) : false;

        $this->getHttpResponse()->setContentType('application/json');
        $this->sendJson(['exists' => $exists]);
    }
    ...
    public function actionOauthCallback(): void
    {
        ...
        $linkUserId = $section->get('link_user_id');

        // DEBUG: Log what we received
        if (!$provider) {
            $this->flashMessage('DEBUG: Provider je prázdný v session. Code=' . ($code ? 'ano' : 'ne') . ', State=' . ($state ? 'ano' : 'ne'), 'warning');
            $this->redirect('Sign:in');
        }

        // If this is account linking AND user is still logged in, redirect to Profile handler
        // Otherwise treat as normal login (linking flag might be stale)
        if ($isLinking && $linkUserId && $this->getUser()->isLoggedIn()) {
        ...
        // Clear linking flags to prevent issues
        $section->remove('linking');
        $section->remove('link_user_id');

        if ($error) {
            $this->flashMessage('Přihlášení bylo zrušeno: ' . $error, 'warning');
            $this->redirect('Sign:in');
        }

        if (!$code || !$state) {
            $this->flashMessage('Neplatná odpověď od poskytovatele. Code=' . ($code ? 'ano' : 'ne') . ', State=' . ($state ? 'ano' : 'ne'), 'error');
            $this->redirect('Sign:in');
        }
        ...
        try {
            ...
            throw $e; // Nette redirect - musí projít dál
        } catch (OAuthException $e) {
